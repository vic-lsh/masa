"""
Tracebench (trace-driven RPC benchmark) application plugin for the experiment runner.

Tracebench differs from the default runner flow:
- It generates a docker-compose.yml per (policy, rps, repeat) via `python -m tracebench.main`
...
from .tracebench_config import TracebenchConfigGenerator
from .utils import normalize_features_to_tag

logger = logging.getLogger(__name__)

GENERIC_SERVICE_IMAGE = "generic_service"
TRACEBENCH_LOADGEN_IMAGE = "tracebench_load_generator"
...
class TracebenchBuilder(AppBuilder):
    """
    Build logic for Tracebench images.

    - Always builds tracebench_load_generator once per runner invocation.
    ...
    def build(
        ...
        **kwargs,
        ...
        # NOTE: rust_log/app_config_path/gen_config_path are unused for Tracebench builds today.
        ...
        dockerfile = (
            repo_root / "apps" / "tracebench" / "generic-service" / "Dockerfile"
        )
        if not dockerfile.exists():
            raise FileNotFoundError(f"Tracebench dockerfile not found: {dockerfile}")
        ...
        if tag not in self._built_loadgen_feature_keys:
            loadgen_cache_id = f"tracebench-loadgen-{tag}"
            loadgen_cmd = [
                "docker",
                "buildx",
                "build",
        ...
        if tag not in self._built_feature_keys:
            ...
            # Format: tracebench-{tag} to match the pattern used by other apps
            cache_id = f"tracebench-{tag}"
...
class TracebenchApp(AppPlugin):
    ...
    def get_required_gen_config_fields(self) -> list[str]:
        # Tracebench needs repeats and RPS sweep values.
        return ["Repeats", "Rps"]
    ...
    def generate_env_vars(
        ...
        # Tracebench env vars are interpreted by the generated docker-compose stack.
        cfg = app_config or {}
        env: dict[str, str] = {}
        ...
        for k, v in extra_env.items():
            env[str(k)] = str(v)
        if extra_env:
            env["TRACEBENCH_SERVICE_EXTRA_ENV_KEYS"] = ",".join(
    ...
    def get_docker_config(self) -> DockerConfig:
        # compose_file is unused for Tracebench (it is generated per run).
        return DockerConfig(
            compose_file="docker-compose.yml",  # Changed from 'generated' to generic, though prepare_workload sets it
            network_name="tracebench_network",
            loadgen_image_name=TRACEBENCH_LOADGEN_IMAGE,
            loadgen_binary_name="tracebench_load_generator",
            app_config_filename="tracebench.json",
        )
    ...
    def prepare_workload(
        ...
        """
        Prepare workload configuration using TracebenchConfigGenerator.
        """
        tracebench_cfg = config.app_config or {}
        ...
        if not rps_values:
            raise ValueError(
                "gen_config.json must include non-empty 'Rps' for Tracebench"
            )
        ...
        env["SLO_MS"] = str(int(tracebench_cfg["slo_ms"]))
        ...
        # via env + volume injection in apps/tracebench/tracebench/orchestrator.py;
        ...
        # Generate Configs
        config_gen = TracebenchConfigGenerator(executor=executor or self.executor)
        ...
        callgraph_dirs_raw = tracebench_cfg.get("callgraph_dirs", [])
        callgraph_dirs = [
    ...
    def _prepare_k8s_workload(
        ...
        for svc_name, svc_info in deploy_data.get("services", {}).items():
            ...
            # names (see apps/tracebench/tracebench/utils.py), so the deployment
        ...
        # appends "-1" in charts/tracebench/templates/services.yaml).
        ...
        callgraph_dirs = [
            ...
            for d in tracebench_cfg.get("callgraph_dirs", [])
        ]

        if not callgraph_dirs:
            raise ValueError(
                "tracebench.json must provide at least one callgraph directory"
            )
        ...
        for cg_dir in callgraph_dirs:
            if not cg_dir.exists() or not cg_dir.is_dir():
                raise FileNotFoundError(
                    f"Tracebench call graph directory missing: {cg_dir}"
                )
    ...
    def get_deployment_location(
        ...
        if use_k8s:
            return repo_root / "charts" / "tracebench", "."
    ...
    def get_loadgen_spec(
        ...
        if policy_params_mount:
            ...
            # tracebench's generic-service Dockerfile doesn't use the shared
            # entrypoint that hotel/socialnet rely on, so set the env var
            # explicitly here — the tracebench-loadgen image uses the same image
        ...
        cmd_str = "tracebench-loadgen && echo 'TRACEBENCH_LOADGEN_DONE'"
        if use_k8s:
            # In K8s, we need to keep the pod running to copy artifacts via exec
            cmd_str += " && sleep infinity"

        return TaskSpec(
            name="tracebench-loadgen",
            image=TRACEBENCH_LOADGEN_IMAGE,
            ...
            wait_for_log_pattern="TRACEBENCH_LOADGEN_DONE",
            cleanup=True,
        )
    ...
    def get_required_images(self, features: Optional[str] = None) -> list[str]:
        ...
        feature_image = _generic_service_image_for_policy(policy)
        images: list[str] = []
        for img in (TRACEBENCH_LOADGEN_IMAGE, feature_image, GENERIC_SERVICE_IMAGE):
    ...
    def verify_results(self, config: "ExperimentConfig") -> bool:
        """
        Verification logic for Tracebench.
        Checks for presence of metadata and latency files, and validates goodput.
        """
        logger.info(f"Verifying Tracebench experiment: {config.experiment_name}")
        ...
        for i in range(repeats):
            for policy in policies:
                # Tracebench structure: {out_dir}/{iteration}/{policy}/run_0/
                ...
                # TracebenchApp.prepare_workload uses output_dir directly.
                # So we should look in {out_dir}/{iteration}/{policy}/ (no run_0)

                # Wait, TracebenchApp used to create run_0. Now it doesn't.
                ...
                if not policy_run_dir.exists():
                    logger.error(
                        f"Tracebench policy run directory missing: {policy_run_dir}"
                    )
                    all_passed = False
                    continue
                ...
                if not (policy_run_dir / "metadata.json").exists():
                    logger.error(
                        f"Tracebench metadata.json missing: {policy_run_dir / 'metadata.json'}"
                    )
                    all_passed = False
                ...
                for rps in rps_list:
                    ...
                    if not latency_files:
                        logger.error(
                            f"Tracebench latency files missing for {rps} RPS in {policy_run_dir}"
                        )
                        all_passed = False
                        continue
                    ...
                    for latency_file in latency_files:
                        try:
                            ...
                            logger.error(
                                f"Failed to read Tracebench CSV {latency_file}: {e}"
                            )
                            all_passed = False
                            continue
