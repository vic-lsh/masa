...
class SynthbenchApp(AppPlugin):
    """
    Plugin for the synthbench benchmark application.

    The synthbench app has a simpler architecture with child services that can
    ...
    def __init__(self):
        """Initialize synthbench app plugin."""
    ...
    def create_builder(self) -> "SynthbenchBuilder":
        return SynthbenchBuilder()
    ...
    def create_gen_config_dict(
        ...
        (synthbench-frontend-service). Docker Compose DNS resolution uses service names,
        ...
        if "Addr" in config:
            ...
            # Use the service name from Docker Compose (synthbench-frontend-service)
            # Docker Compose DNS resolution uses service names, not container names
            # The container will be named {project_name}-synthbench-frontend-service-1
            # but DNS resolution uses the service name

            service_name = service_name_override or "synthbench-frontend-service"
    ...
    def create_call_graph_compose_dict(
        ...
        services["synthbench-frontend-service"] = {
            "image": f"synthbench_frontend:{image_tag}",
            "restart": "always",
            "networks": ["synthbench_network"],
        ...
        for service_def in call_graph["services"]:
            ...
            services[service_name] = {
                "image": f"synthbench_child:{image_tag}",
                "scale": replicas,
                "networks": ["synthbench_network"],
                "environment": [
                    "BINARY_NAME=synthbench_child",
                    "LOG_LEVEL=${LOG_LEVEL:-info}",
        ...
        compose_content = {
            "services": services,
            "networks": {"synthbench_network": {"driver": "bridge"}},
        }
    ...
    def generate_env_vars(
        ...
        """
        Generate environment variables for synthbench application.
        ...
        if app_config:
            ...
            env_vars["SYNTHBENCH_DISABLE_CPU_YIELD"] = "0"
    ...
    def get_docker_config(self) -> DockerConfig:
        """
        Return Docker configuration for synthbench application.
        ...
        return DockerConfig(
            ...
            network_name="local_synthbench_network",
            loadgen_image_name="synthbench_client_bench:<features>",
            loadgen_binary_name="synthbench_client_bench",
    ...
    def get_required_images(self, features: Optional[str] = None) -> list[str]:
        ...
        return [
            f"synthbench_frontend{suffix}",
            f"synthbench_child{suffix}",
            f"synthbench_client_bench{suffix}",
        ]
    ...
    def get_container_names(
        ...
        """
        Get list of container names for synthbench application.
        ...
        # Docker Compose will create: {project}-synthbench-frontend-service-1
        container_names = []
    ...
    def get_deployment_location(
        ...
        if use_k8s:
            return repo_root / "charts/synthbench", "."
        ...
        docker_config = self.get_docker_config()
        # Default scripts dir
        return repo_root / "exp/synthbench/scripts", docker_config.compose_file
    ...
    def get_loadgen_spec(
        ...
        # The legacy classes (SynthbenchLoadGenerator) are still useful for names/images

        # Determine image and binary
        tag = self.get_image_tag(features)
        image = (
            f"synthbench_client_bench:{tag}"
            if tag
            else "synthbench_client_bench:latest"
        )
        binary = "synthbench_client_bench"

        # Network
        network = (
            f"{project_name}_synthbench_network"
            if project_name
            else "local_synthbench_network"
        )
        if use_k8s:
        ...
        # For synthbench, we might need some from the main env_vars?
        ...
        return TaskSpec(
            name=f"{project_name}-loadgen" if project_name else "synthbench-loadgen",
            image=image,
            env_vars=task_env,
            network=network,
            volumes=volumes,
...
class SynthbenchBuilder(AppBuilder):
    """
    Build logic for the synthbench app docker images.
    ...
    The synthbench app requires three separate docker images:
    - synthbench_frontend:latest - frontend service
    - synthbench_child:latest - child service (can be scaled)
    - synthbench_client_bench:latest - load generator
    """
    ...
    def build(
        ...
        *,
        ...
        logger.info(
            f"Building {len(binaries)} docker images for synthbench app using multi-stage build"
        )
        if features:
        ...
        if gen_config_path is None:
            raise ValueError("gen_config_path is required for synthbench app")
        ...
        try:
            ...
            # logger.info("Stage 1: Building all binaries for synthbench app")
            logger.info("Stage 1: Building all binaries for synthbench app")
            ...
            for binary_name in binaries:
                ...
                # Image name: synthbench_<binary>:<tag> or synthbench_<binary>:latest if no features
                if tag:
        ...
        logger.info("All synthbench app docker images built successfully")
        return None
