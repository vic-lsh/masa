"""
Tracebench configuration generation logic.
"""
...
class TracebenchConfigGenerator:
    """Helper to generate Tracebench docker-compose and deployment configs."""
    ...
    def generate(
        ...
        *,
        ...
        Args:
            app_dir: Directory containing Tracebench app code (where tracebench module is)
            output_dir: Directory to write generated files
            gen_config: Content of gen_config.json
            app_config: Content of tracebench.json
        ...
        if not callgraph_dirs_raw or not isinstance(callgraph_dirs_raw, list):
            raise ValueError(
                "tracebench.json must include 'callgraph_dirs' (list of call graph directory paths)"
            )
        ...
        for callgraph_dir in callgraph_dirs:
            if not callgraph_dir.exists():
                raise FileNotFoundError(
                    f"Tracebench callgraph_dir does not exist: {callgraph_dir}"
                )
        ...
        if gen_proc.returncode != 0:
            raise RuntimeError(
                f"Tracebench config generation failed ({gen_proc.returncode}). See log at {log_path}"
            )
