"""
Plotting utilities for Tracebench experiment results.
"""
...
def _parse_rps_from_filename(path: Path) -> float:
    """Parse RPS value from known Tracebench CSV filename patterns."""
...
def _plot_early_return_timeline(
    ...
    *,
    ...
    """Plot early-return rate over time, stitching RPS periods in their original run order."""
    fig, ax = plt.subplots(figsize=(14, 6))
    cmap = plt.get_cmap("tab10")
...
def _resolve_rps_values(
    ...
    raise FileNotFoundError("No RPS values found for Tracebench output")
...
def generate_plots(
    ...
    *,
    ...
    """Render the tracebench plot pipeline, honoring the same selector flags as
    the regular hotel/socialnet/synthbench path.
    ...
    """
    # Lazy import to avoid an `all` ↔ `tracebench` import cycle.
    ...
    # use_cache is honored via load_plot_data in the regular path; tracebench
    ...
    tracebench_config = _load_json(config_dir / "tracebench.json")
    ...
    slo_ms = float(tracebench_config.get("slo_ms", 0) or 0)
    ...
    if per_iteration_goodput:
        ...
        if plot_goodput:
            ...
            # hotel/socialnet/synthbench goodput_<API>.png plots; the helper
