...
def _compute_goodput(
    ...
    # Align with synthetic/hotel goodput semantics when request traces provide error strings.
    if "error" in df.columns:
        df = filter_excluded_errors(df)

    # For root-latency exports that include explicit error flag, also exclude flagged rows.
