...
class TestOrchestration:
    ...
    def mock_repo_root(self, tmp_path):
        ...
        root = tmp_path / "repo"
        root.mkdir()
        (root / "apps" / "synthbench").mkdir(parents=True)
        (root / "exp" / "synthbench" / "in" / "test_exp").mkdir(parents=True)
        ...
        gen_config_path = (
            root / "exp" / "synthbench" / "in" / "test_exp" / "gen_config.json"
        )
    ...
    def test_experiment_dry_run_commands(self, mock_repo_root):
        ...
        config = ExperimentConfig(
            ...
            exp_dir=mock_repo_root / "exp" / "synthbench",
            app_dir=mock_repo_root / "apps" / "synthbench",
            in_dir=mock_repo_root / "exp" / "synthbench" / "in" / "test_exp",
            out_dir=mock_repo_root / "exp" / "synthbench" / "out" / "test_exp",
            plot_dir=mock_repo_root / "exp" / "synthbench" / "plots" / "test_exp",
        ...
        # Stage 1: builder
        assert any(
            "target builder" in cmd and "synthbench" in cmd for cmd in cmd_strings
        )
        # Stage 2: runtime-base
        assert any("target runtime-base" in cmd for cmd in cmd_strings)
        # Stage 3: runtime (binaries)
        assert any(
            "target runtime" in cmd and "synthbench_frontend" in cmd
            for cmd in cmd_strings
        )
        assert any(
            "target runtime" in cmd and "synthbench_child" in cmd for cmd in cmd_strings
        )
