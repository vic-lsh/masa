...
def test_tracebench_run_workload_orchestration(
    tmp_path, mock_executor, mock_deployment
):
    """
    Test that TracebenchApp.run_workload generates the correct sequence of commands:
    1. Build images
    2. Generate docker-compose via tracebench.main
    3. Run docker compose up
    4. Wait for loadgen
    5. Cleanup
    """
    app = TracebenchApp(executor=mock_executor)

    # Setup test directory structure
    repo_root = tmp_path / "repo"
    repo_root.mkdir()

    app_dir = repo_root / "apps" / "tracebench"
    ...
    in_dir = repo_root / "exp" / "tracebench" / "in" / "test_exp"
    ...
    assert any("tracebench_load_generator" in str(cmd) for cmd in commands)
    # Should build generic service
    assert any("generic_service" in str(cmd) for cmd in commands)

    # 2. Check tracebench generation command
    gen_cmd = next(
        (
            cmd
            for cmd in commands
            if "tracebench.main" in str(cmd)
            or ("python" in cmd[0] and "tracebench.main" in cmd)
        ),
        None,
    )
    ...
    assert "tracebench-test-exp-" in kwargs["project_name"]
    ...
    assert task_spec.name == "tracebench-loadgen"


def test_tracebench_orchestration_k8s(tmp_path, mock_executor):
    ...
    in_dir = repo_root / "exp" / "tracebench" / "in" / "test_exp_k8s"
    in_dir.mkdir(parents=True)
    ...
    # Check that it installed the chart from charts/tracebench
    start_args = real_k8s_manager.start.call_args[1]
    assert "charts/tracebench" in str(
        start_args.get("app_dir", "")
    ) or "tracebench" in str(start_args.get("deployment_config", ""))

    # 3. Should run loadgen task
    real_k8s_manager.run_task.assert_called()
    task_spec = real_k8s_manager.run_task.call_args[0][0]
    assert task_spec.image == "tracebench_load_generator"
    assert task_spec.name == "tracebench-loadgen"
