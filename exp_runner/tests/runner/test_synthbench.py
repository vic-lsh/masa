"""
Tests for the synthbench application module.

This module tests the synthbench app functionality including:
...
from exp_runner.runner.apps.synthbench import (
    SynthbenchApp,
    SynthbenchBuilder,
)
...
class TestSynthbenchBuilder:
    """Tests for SynthbenchBuilder with feature-based tags and cache ID consistency."""

    @patch("exp_runner.runner.apps.synthbench.subprocess.run")
    @patch("exp_runner.runner.apps.synthbench.logger")
...
class TestSynthbenchApp:
    """Tests for SynthbenchApp integration with feature-based tags."""
    ...
    def test_get_loadgen_spec_with_features(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert task_spec.image == "synthbench_client_bench:policy-x-policy-y"
            assert task_spec.name == f"{project_name}-loadgen"
            assert task_spec.network == f"{project_name}_synthbench_network"
    ...
    def test_get_loadgen_spec_without_features(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert isinstance(task_spec, TaskSpec)
            assert task_spec.image == "synthbench_client_bench:latest"
    ...
    def test_create_builder(self):
        ...
        assert isinstance(builder, SynthbenchBuilder)
