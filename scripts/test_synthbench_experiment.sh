...
exp_dir="$repo_root/exp/synthbench"
exp_names=()
no_cache=""
deploy_mode="docker"
deploy_args=""
...
if ! command -v docker >/dev/null 2>&1; then
    echo "Docker is required to run the synthbench experiment test." >&2
    exit 1
fi
...
if [ ! -d "$exp_dir" ]; then
    echo "Synthbench experiment directory not found at $exp_dir" >&2
    exit 1
fi
...
run_test() {
    ...
    echo "Running synthbench experiment: $exp_name"
    cd "$repo_root"
    # We use unquoted variables for flags to allow empty strings to disappear
    # shellcheck disable=SC2086
    python -m exp_runner.runner run synthbench "$exp_name" $no_cache $deploy_args --smoke-test --plot
...
echo "All synthbench CI experiment tests passed."
