...
out_dir="$repo_root/exp/tracebench/out/$experiment_name"
deploy_mode="docker"
deploy_args=""
...
if ! command -v docker >/dev/null 2>&1; then
    echo "Docker is required to run the Tracebench experiment test." >&2
    exit 1
fi

if ! command -v cargo >/dev/null 2>&1; then
    echo "Cargo is required to run the Tracebench experiment test." >&2
    exit 1
fi
...
echo "Running Tracebench experiment using exp_runner.runner with mode: $deploy_mode"
# shellcheck disable=SC2086
python -m exp_runner.runner run tracebench "$experiment_name" --rm-data --smoke-test $deploy_args

echo "Verifying Tracebench outputs and metadata mode ($expected_mode)..."
...
config = ExperimentConfig.load(experiment, "tracebench", repo_root, app)

if not app.verify_results(config):
    raise SystemExit("Tracebench smoke test verification failed")
...
if errors:
    ...
    raise SystemExit("Tracebench metadata mode validation failed")

print("Tracebench metadata mode validated for", expected_mode)
PY

echo "Tracebench experiment smoke test passed."
