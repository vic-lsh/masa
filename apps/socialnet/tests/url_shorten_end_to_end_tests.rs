...
async fn setup_test_server(
    ...
    UrlShortenServiceClient<tonic::transport::Channel>,
) {
...
async fn test_compose_urls() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for url in &result.urls {
        assert!(
            url.shortened_url.starts_with("http://short-url/"),
            "Shortened URL has unexpected format: {}",
            url.shortened_url
        );
        assert_eq!(
            url.shortened_url.len(),
            "http://short-url/".len() + 10,
            "Shortened URL has unexpected length: {}",
            url.shortened_url
        );
    }
...
async fn test_get_extended_urls() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(
        get_result.exception.is_none(),
        "Unexpected exception: {:?}",
        get_result.exception
    );
    assert_eq!(
        get_result.expanded_urls.len(),
        1,
        "Expected 1 URL, got {}",
        get_result.expanded_urls.len()
    );
    assert_eq!(
        get_result.expanded_urls[0], *original_url,
        "Got unexpected expanded URL: {}",
        get_result.expanded_urls[0]
    );
...
async fn test_nonexistent_url() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Some(exception) = get_result.exception {
        assert!(
            ...
            "Expected error about URL not found, got: {}",
            exception.message
        );
    }
...
async fn test_empty_input() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(
        compose_result.exception.is_none(),
        "Unexpected exception for empty input: {:?}",
        compose_result.exception
    );
    assert_eq!(
        compose_result.urls.len(),
        0,
        "Expected empty result, got {} URLs",
        compose_result.urls.len()
    );
...
async fn test_full_workflow() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let shortened_urls: Vec<String> = compose_result
        .urls
        .iter()
