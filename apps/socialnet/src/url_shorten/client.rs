...
impl UrlShortenClient {
    ...
    pub async fn compose_urls(
        ...
        let request = Request::new(ComposeUrlsRequest { req_id, urls });
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match client.compose_urls(req_id, urls).await {
        Ok(url_list) => {
            ...
            let shortened_urls: Vec<String> = url_list
                .iter()
            ...
            match client.get_extended_urls(req_id, shortened_urls).await {
                Ok(expanded_urls) => {
                    println!(
                        "\nSuccessfully retrieved {} expanded URLs:",
                        expanded_urls.len()
                    );
