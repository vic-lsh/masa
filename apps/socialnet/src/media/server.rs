...
use crate::media::{
    ComposeMediaRequest, ComposeMediaResponse, Media, ServiceException, ErrorCode,
    ...
    async fn compose_media(
        ...
        if req.media_types.len() != req.media_ids.len() {
            let exception = ServiceException {
                ...
                message: "The lengths of media_id list and media_type list are not equal".to_string(),
            };
        ...
        let media = req.media_types.iter().zip(req.media_ids.iter())
...
mod tests {
    ...
    async fn start_test_server() -> (String, oneshot::Sender<()>) {
        ...
        let listener = std::net::TcpListener::bind("[::1]:0").expect("Failed to bind to random port");
    ...
    async fn test_compose_media_success() {
        ...
        let mut client = MediaServiceClient::connect(server_addr).await.expect("Failed to connect");
        ...
        assert!(response_inner.exception.is_none(), 
            "Received unexpected exception: {:?}", response_inner.exception);
    ...
    async fn test_compose_media_validation_error() {
        ...
        assert!(response_inner.exception.is_some(), "Expected validation exception");
        if let Some(exception) = response_inner.exception {
            assert!(exception.message.contains("lengths"), 
                "Expected error about lengths mismatch, got: {}", exception.message);
        }
    ...
    async fn test_empty_lists_case() {
        ...
        assert!(response_inner.exception.is_none(), "Expected no exception with empty lists");
