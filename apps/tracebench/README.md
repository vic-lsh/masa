...
The recommended way to run Tracebench experiments is using the experiment runner script. This script automates the process of building Docker images, running experiments across multiple policies and RPS values, and organizing output data.
...
1. Create an experiment directory under `exp/tracebench/data/in/<experiment_name>/` with the following files:
   ...
   **`tracebench.json`** - Tracebench-specific configuration:
   ```json
   {
...
$ uv run -m exp_runner run tracebench <experiment_name>
```

Or with automatic plot generation:
```bash
$ uv run -m exp_runner run tracebench <experiment_name> --plot
```
...
- `config_dir`: Path to the tracebench config directory (relative to repo root)
...
See `scripts/test_tracebench_experiment.sh` for a complete example of running an experiment and validating the output.
...
```bash
$ uv run -m exp_runner plot tracebench <experiment_name>
```
