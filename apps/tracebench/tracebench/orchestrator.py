...
def generate_service_configs(
    ...
    for callgraph_dir in callgraph_dirs:
        ...
        replicas = replay_cfg.replicas.count_for(FRONTEND_SERVICE_NAME)
    ...
    for service_name in all_services:
        ...
        if service_name not in added_services:
            replicas = replay_cfg.replicas.count_for(service_name)
...
def _merge_callgraph_directories(
    ...
    for callgraph_dir in callgraph_dirs:
        graph_id = (
            callgraph_dir.name
        )  # Use directory name as graph_id (e.g., "S_14677443")
...
def generate_docker_compose(
    ...
    if not os.environ.get("TRACEBENCH_SKIP_LOADGEN"):
...
def _make_service_def(
    ...
    replica_lookup_name = (
        original_service_name if original_service_name is not None else service_name
    )
    return ComposeService(
        image=image_name,
        scale=replay_cfg.replicas.count_for(replica_lookup_name),
        deploy={
            "resources": {
                "limits": {
...
def _make_environment_def(
    service_name: str, svc_port: int, callgraph_dirs: list[Path]
) -> dict[str, str]:
    # For user-* services, set SERVICE_NAME to "USER" instead of the deployment service name
    if service_name == FRONTEND_SERVICE_NAME or service_name.startswith(
        FRONTEND_SERVICE_NAME + "-"
    ):
    ...
    environment.update(_collect_tracebench_service_extra_env())
    ...
    # in-container mount path so libs/masa-policy can load it. tracebench's
...
def _make_volumes_def(
    callgraph_dirs: list[Path], deployment_output_path: Path
) -> list[str]:
    volumes = []
...
def _make_load_generator_config_yaml(
    ...
    environment.update(
        _collect_optional_env(
            "DURATION", "RPS", "RPS_VALUES", "MAX_IN_FLIGHT", "STATS_INTERVAL_SEC"
        )
    )
    ...
    return ComposeService(
        image="tracebench_load_generator",
...
def _collect_tracebench_service_extra_env() -> dict[str, str]:
    """Forward tracebench.json extra_env values to generic-service containers."""
    raw_keys = os.environ.get("TRACEBENCH_SERVICE_EXTRA_ENV_KEYS", "")
