...
class ReplayConfigError(RuntimeError):
    """Raised when tracebench configuration cannot be parsed."""
...
class ReplicaConfig:
    ...
    def from_file(cls, path: Path) -> "ReplicaConfig":
        ...
        try:
            ...
            raise ReplayConfigError(f"Failed to parse replica config {path}: {exc}") from exc
...
class ReplayConfig:
    ...
    @classmethod
    def from_replicas_path(cls, replicas_path: Optional[Path]) -> "ReplayConfig":
