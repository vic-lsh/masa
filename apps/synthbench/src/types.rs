// Strong types for synthbench application
...
impl ServiceId {
    pub fn new(id: impl Into<String>) -> Result<Self, SynthbenchError> {
        let id = id.into();
        if id.trim().is_empty() {
            return Err(SynthbenchError::InvalidServiceId(
...
impl MethodName {
    pub fn new(name: impl Into<String>) -> Result<Self, SynthbenchError> {
        let name = name.into();
        if name.trim().is_empty() {
            return Err(SynthbenchError::InvalidMethodName(
...
impl Probability {
    pub fn new(p: f64) -> Result<Self, SynthbenchError> {
        if p < 0.0 || p > 1.0 {
            return Err(SynthbenchError::InvalidProbability(format!(
...
impl DurationMicros {
    pub fn new(us: u64) -> Result<Self, SynthbenchError> {
        if us == 0 {
            return Err(SynthbenchError::InvalidDuration(
...
use crate::error::SynthbenchError;
use rand::Rng;
