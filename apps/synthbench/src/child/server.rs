...
use crate::config::{parse_call_sequences, ServiceMethod, SynthbenchConfig};
    ...
    pub async fn new(config: SynthbenchConfig) -> Self {
        ...
        let current_service_id = std::env::var("SERVICE_ID")
            ...
            .or_else(|| {
                ...
                // In docker compose, hostname might be like "synthbench-child-service-1"
...
impl Child for ChildImpl {
    async fn run_synthbench(
        &self,
        _request: Request<child::RunSynthbenchRequest>,
    ) -> Result<Response<child::RunSynthbenchResponse>, Status> {
