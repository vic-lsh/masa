...
impl LoadGenerator {
    pub fn new(
        ...
        let gen_tokens = Arc::new(AtomicI32::new(max_concurrent_reqs.try_into().unwrap()));
        Self {
            gen_cfg,
            rng,
            rps,
    ...
    async fn run(&mut self) -> Result<(), Box<dyn Error>> {
        ...
        tokio::task::spawn(async move {
            ...
            let avg_goodput = good_total / secs;
            log::warn!("goodput per sec: {}", avg_goodput);
        });
        ...
        loop {
            ...
            // try to get token to generate request
            let res = self.gen_tokens.fetch_sub(1, Ordering::SeqCst);
            if res <= 0 {
            ...
            if api == "Search" {
                ...
                let gen_tokens = self.gen_tokens.clone();
                tokio::task::spawn(async move {
                    ...
                    // println!("search done");
                    gen_tokens.fetch_add(1, Ordering::SeqCst);
                });
            ...
            //             }
            //         }
            //         gen_tokens.fetch_add(1, Ordering::Relaxed);
            //     });
            } else {
