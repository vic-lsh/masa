...
struct BudgetState {
    ...
    er_ema: f64,
    /// Once true, budget stays engaged until the next idle gap.
    budget_engaged: bool,
}
...
pub(crate) struct AdmissionController {
    ...
    /// Lock-free accumulator: count of early-returned completions at ingress.
    er_count: AtomicU64,
    /// Lock-free accumulator: count of all completions at ingress (success + ER).
...
impl AdmissionController {
    ...
    /// Record an early-returned completion at ingress (lock-free).
    ...
    ///
    /// Uses ER-rate tracking with a one-way latch: once the ER rate exceeds
    /// the threshold, budget-based admission engages and stays engaged until
    /// an idle gap resets it.
    pub(crate) fn should_admit(&self, est_child_cost: u64) -> bool {
        ...
        // Idle gap detection: reset latch and er_ema for clean start;
        // preserve goodput_rate (accurate capacity from previous step).
        let idle_threshold = 0.5;
        if elapsed > idle_threshold {
            state.er_ema = 0.0;
            state.budget_engaged = false;
            // Skip EMA updates — stale data from gap.
        } else if elapsed > 0.0 {
        ...
        // Check if ER rate crosses threshold — engage budget (one-way latch).
        if state.er_ema > p.rejection_threshold {
            state.budget_engaged = true;
        }

        // If budget is not engaged: admit freely (no budget check).
        if !state.budget_engaged {
            return true;
        }

        // Budget is engaged: use goodput-tracking token bucket.
...
impl PredictiveAdmission {
    ...
    /// Record an early-returned completion at ingress.
    #[inline]
    pub(crate) fn record_early_return(&self) {
