...
impl Layer for PredAdmissionLayer {
    ...
    fn after_child_rpc<T>(
        ...
        // Accumulate completion tracking for goodput and ER rate (ingress only).
        #[cfg(feature = "ac_pred")]
        if ctx.hop_count() == 0 {
            if response.is_ok() {
...
impl AdmissionController {
    ...
    ///
    /// Uses ER (early-return) rate from abort_slo as the mode-switching
    /// signal. When ER rate is low, admits freely. When ER rate exceeds
    /// the threshold, engages goodput-tracking token bucket.
    pub(crate) fn should_admit(&self, est_child_cost: u64) -> bool {
        ...
        // Skip EMA updates during idle gaps (preserves state across load transitions).
        let idle_threshold = 0.5;
        if elapsed > 0.0 && elapsed <= idle_threshold {
        ...
        // If ER rate is below threshold: admit freely, no budget check.
        if state.er_ema <= p.rejection_threshold {
            return true;
        }

        // ER rate above threshold: engage goodput-tracking budget.
