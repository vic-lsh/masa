...
use crate::masa::{Hooks, ParentHooks};
use crate::{
    body::BoxBody,
    ...
    T: Codec,
{
    ...
    /// Handle a single unary gRPC request.
    pub async fn masa_unary<S, B, M>(
        &mut self,
        mut service: S,
        req: http::Request<B>,
        req_ctx: M::ParentContext,
    ) -> http::Response<BoxBody>
    where
        S: UnaryService<T::Decode, Response = T::Encode>,
        B: Body + Send + 'static,
        ...
        M: Hooks,
    {
        ...
        let fut = service
            ...
            .abortable()
            .before_poll(|| {
                crate::masa::server::set_parent_ctx::<M>(req_ctx.as_ref());
            ...
            .after_poll(|poll| {
                crate::masa::server::reset_parent_ctx::<M>();
