...
1. **Activate the virtual environment** (recommended for interactive use):
   ...
   python3 -m exp_runner plot hotel exp1
   ```

2. **Use `uv run`** (no activation needed):
   ```bash
   uv run python3 -m exp_runner plot hotel exp1
   ```
...
uv run python -m exp_runner plot <app> <experiment-name>

# For example:
uv run python -m exp_runner plot hotel exp1
uv run python -m exp_runner plot synthetic quick_test
uv run python -m exp_runner plot mssim e2e_test
uv run python -m exp_runner plot socialnet exp1
```
...
# For example:
uv run python -m exp_runner run hotel exp1 --plot
uv run python -m exp_runner run synthetic quick_test --plot
uv run python -m exp_runner run socialnet exp1 --plot

# Run multiple experiments sequentially
uv run python -m exp_runner run-multiple hotel "exp1 exp2 exp3" --plot
```
